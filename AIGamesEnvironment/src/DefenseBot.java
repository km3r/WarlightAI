...
/*
 * @Author Jason
 *  Want to have at least 2 armies in the territory before it expands
 *  Will attack when chance of winning is equal to or greater than 0.65 (attacks more often than the default winProb)
 */
public class DefenseBot extends GameBot
{
	...
	public DefenseBot() {
		...
		System.err.println("---==[ DefenseBot BOT INITIALIZED ]==---");
	}
	...
	public List<PlaceCommand> placeArmies(long timeout) {
		...
		// SCORE 0 means all the neighbors are of the same troop (my guys)
		int i = 0;
	...
	public List<MoveCommand> moveArmies(long timeout) {
		...
		for (RegionState from : state.me.regions.values()) {
			for (RegionState to : from.neighbours) {
				...
				// When attacking a different continent, only attack when you have 5 more than RequiredSoldiersToConquerRegion
				if(from.region.continent.id != to.region.continent.id && from.armies >= 3 + getRequiredSoldiersToConquerRegion(from, to, 0.7)) {
	...
	private int getRequiredSoldiersToConquerRegion(RegionState from, RegionState to, double winProbability) {
		...
		for (int a = defenders; a <= attackers; ++a) {
			...
			chance += 0.05;
			// when chance is equal to or more than 0.65, attack
	...
	private MoveCommand transfer(RegionState from, RegionState to) {
		MoveCommand result = new MoveCommand(from.region, to.region, from.armies-2);
		return result;
	}
	...
	public static void runInternal() {
		...
		config.bot1Init = "internal:conquest.bot.playground.DefenseBot";
		...
		config.bot2Init = "internal:conquest.bot.playground.HeuristicBot";
	...
	public static void runExternal() {
		BotParser parser = new BotParser(new DefenseBot());
